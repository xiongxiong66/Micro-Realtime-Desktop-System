...
#define SETTINGS_RECORD_SIZE  21U
...
void Set_Sys_Load(void)
{
    ...
    if (SFlash_Init() == SFLASH_OK
     && SFlash_Read(addr, (uint8_t *)&rec, SETTINGS_RECORD_SIZE) == SFLASH_OK
...
void Set_Sys_Save(void)
{
    ...
    if (SFlash_EraseSector(SETTINGS_SECTOR_BASE) != SFLASH_OK)
    {
        Monitor_Sys_ReportError();
        return;
    }
    if (SFlash_Write(addr, (const uint8_t *)&rec, SETTINGS_RECORD_SIZE) != SFLASH_OK)
    {
        Monitor_Sys_ReportError();
    }
}
...
static void Set_Change(uint8_t sel, int8_t dir)
{
    ...
    max_level = (sel == 3U) ? 5U : SETTINGS_LEVELS;
    if (dir > 0)
    {
...
void Set_Sys_Run(void)
{
    ...
    for (;;)
    {
        ...
        {
            case '2':
                sel = (sel > 0U) ? (uint8_t)(sel - 1U) : 4U;
                break;
            case '8':
                sel = (sel + 1U < 5U) ? (uint8_t)(sel + 1U) : 0U;
                break;
            case '4':
                if (sel < 4U) Set_Change(sel, -1);
                break;
            case '6':
                if (sel < 4U) Set_Change(sel, 1);
                break;
