...
static osThreadId_t taskwatch_handles[TASKWATCH_COUNT] = {NULL, NULL, NULL};
static const char taskwatch_names[TASKWATCH_COUNT][6] = {
    "MKey", "SwAdc", "Log"
};
...
void TaskWatch_Check(void)
{
    ...
    for (uint8_t i = 0U; i < TASKWATCH_COUNT; i++)
    {
        ...
        if (eTaskGetState((TaskHandle_t)taskwatch_handles[i]) == eSuspended)
        {
            taskwatch_hang[i] = 0U;
            continue;
        }
