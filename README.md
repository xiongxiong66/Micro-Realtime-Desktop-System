...
- 每个受监控任务带一个“检查开关”：正常运行或处理事件时开启，进入合法等待时关闭。
- UI 等按键通过 `Ui_KeyGet()` 统一封装，等待前关闭检查，收到按键后重新开启并刷新心跳。
- TaskWatch 只检查开关开启且心跳超时的任务；挂起任务和合法等待任务不判 HANG。
...
- `TASK VIEW` 第 1 页显示系统汇总，之后按页显示任务状态；`2/8` 翻任务页，`4/6` 切换每次 B 的快照，`0` 删除当前快照，`*` 返回菜单
...
| `9` | 调试用：让当前 Oled 或 File 任务进入死循环，触发 HANG/看门狗检测（命名和密码页除外） |

> `9` 是异常注入键：正常页面按下后会让当前 UI 任务停止心跳，并立即通知 TaskWatch；写出 `HANG` 后立即刷盘，随后由 IWDG 复位。仅在测试任务异常检测时使用。
