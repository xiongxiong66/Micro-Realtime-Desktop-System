...
- 任务列表按任务名排序显示，位置固定
- Monitor 页面会叠加显示当前摇杆光标位置，光标移动时只局部上传光标区域
- 队列页显示 `CUR`（光标）、`KEY`（按键）、`LOG`（日志）的当前消息数/容量和剩余空间，`CUR` 行约 25ms 高频刷新，`KEY/LOG` 行约 1s 更新
...
```text
RAM:   18464 B / 20 KB   (90.16%)
FLASH: 60340 B / 64 KB   (92.07%)
```
